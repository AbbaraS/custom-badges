...
function isBlankBadge(b: BadgeDefinition): boolean {
	return !b.key.trim() && !b.label.trim() && !b.icon.trim() && !b.color.trim()
		&& b.placeholder === 'default' && !b.placeholderText.trim();
}
...
class BadgesSettingTab extends PluginSettingTab {
	...
	// Discard rows left completely empty when the settings window closes.
	hide(): void {
		const badges = this.plugin.settings.badges;
		const kept = badges.filter((b) => !isBlankBadge(b));
		if (kept.length !== badges.length) {
			this.plugin.settings.badges = kept;
	...
	display(): void {
		...
		settings.badges.forEach((badge, index) => {
			...
			row.addText((text) => {
				...
				this.keyInputs[index] = text.inputEl;
			})
			row.addText((text) => {
		...
		new Setting(containerEl)
			.addButton((btn) => btn
				...
				.setCta()
				.onClick(async () => {
					// Only one unfinished badge at a time: send the user back to it.
					const unfinished = settings.badges.findIndex((b) => !b.key.trim());
					if (unfinished >= 0) {
						new Notice('Give the new badge a key before adding another.');
						const input = this.keyInputs[unfinished];
